...
# Завжди перевіряємо оновлення залежностей (важливо для нових пакетів)
...
# Перевіряємо різні шляхи для крос-платформності (хоча це bash, venv структура може відрізнятись)
if [ -f "python-core/venv/bin/activate" ]; then
    source python-core/venv/bin/activate
elif [ -f "python-core/venv/Scripts/activate" ]; then
    source python-core/venv/Scripts/activate
fi
