...
async function applySessionDefaults(session: Session, sessionsRoot: string): Promise<{ normalized: Session; changed: boolean }>
 {
  ...
  // --- Нові налаштування для Auto-Prompts (Sora 9 Style) ---
  setIfEmpty('enableAutoPrompts', false);
  setIfEmpty('promptDelayMs', 0); // 0 = використовувати глобальний
  setIfEmpty('postLastPromptDelayMs', 120000); // 2 хвилини за замовчуванням
  setIfEmpty('maxPromptsPerRun', 10); // Ліміт на один прогін
