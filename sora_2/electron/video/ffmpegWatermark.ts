...
export async function pickSmartPreviewFrames(videoPath: string, count: number): Promise<string[]> {
  ...
  await new Promise<void>((resolve, reject) => {
    ffmpeg(videoPath)
      .on('end', () => resolve())
      .on('error', (err) => reject(err))
      .screenshots({ timestamps, folder: frameDir, filename: 'frame-%i.png' });
  });
...
export type DetectedZone = { frame: string; zones: BlurZone[] };

export async function detectWatermarkOnFrames(frames: string[], _templatePath: string): Promise<DetectedZone[]> {
  // Placeholder for future detection integration
