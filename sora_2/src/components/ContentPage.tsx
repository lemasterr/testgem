...
export const ContentPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      ...
      try {
        ...
        // Використовуємо єдиний API 'files', бо 'sessionFiles' було видалено під час рефакторингу
        const filesApi = api?.files;
        if (!filesApi?.read) {
          setError('Files API is not available. Please run inside the Electron app.');
          setLoading(false);
          return;
        }
        const response = await filesApi.read(selectedProfile);
        if (!response?.ok) {
  ...
  const handleSave = async (auto = false) => {
    ...
    try {
      ...
      const api = (window as any).electronAPI;
      const saveFn = api?.files?.save;
      if (!saveFn) {
        throw new Error('Files API is not available. Please run inside the Electron app.');
      }
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-white/5 bg-zinc-900/60 p-4 shadow-lg shadow-blue-500/10 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-3">
            ...
            <button
              onClick={async () => {
                try {
                  // Updated to use the unified files API
