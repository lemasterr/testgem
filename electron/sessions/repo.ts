// sora_2/electron/sessions/repo.ts
...
const PROMPTS_DIR_NAME = 'prompts'; // Общая папка для промптов в корне
...
// Хелпер для нормализации имени профиля, идентичный тому, что в profileFiles.ts
...
async function applySessionDefaults(session: Session, sessionsRoot: string): Promise<{ normalized: Session; changed: boolean }>
 {
  ...
  // --- ЛОГИКА СИНХРОНИЗАЦИИ С РЕДАКТОРОМ ---
  // Если у сессии есть профиль Chrome, мы хотим по умолчанию использовать файлы этого профиля.
  const profileName = session.chromeProfileName || 'Default';
  const safeProfileName = normalizeProfileNameForFile(profileName);

  // Пути к общим файлам (как в редакторе ContentPage)
  // Они лежат в <sessionsRoot>/prompts/<Profile>_prompts.txt
  const sharedPromptsRel = path.join(PROMPTS_DIR_NAME, `${safeProfileName}_prompts.txt`);
  const sharedImagesRel = path.join(PROMPTS_DIR_NAME, `${safeProfileName}_images.txt`);
  const sharedTitlesRel = path.join(PROMPTS_DIR_NAME, `${safeProfileName}_titles.txt`);
  ...
  // Если путь к файлам не задан, ставим путь к ОБЩЕМУ файлу профиля
  setIfEmpty('promptsFile', sharedPromptsRel);
  setIfEmpty('imagePromptsFile', sharedImagesRel);
  setIfEmpty('titlesFile', sharedTitlesRel);

  // Остальные файлы (логи, загрузки) оставляем изолированными в папке сессии
  ...
  setIfEmpty('postLastPromptDelayMs', 120000);
  setIfEmpty('maxPromptsPerRun', 10);
  ...
  // Создаем общую папку prompts
  await ensureDir(path.join(sessionsRoot, PROMPTS_DIR_NAME));
...
function toManagedSession(session: Session, stats?: Partial<Pick<ManagedSession, 'promptCount' | 'titleCount' | 'hasFiles'>>): ManagedSession {
  return {
    ...
    enableAutoPrompts: session.enableAutoPrompts,
    promptDelayMs: session.promptDelayMs,
    postLastPromptDelayMs: session.postLastPromptDelayMs,
    maxPromptsPerRun: session.maxPromptsPerRun,
    autoChainAfterPrompts: session.autoChainAfterPrompts,
...
function fromManagedSession(session: ManagedSession): Session {
  return {
    ...
    enableAutoPrompts: session.enableAutoPrompts ?? false,
    promptDelayMs: session.promptDelayMs ?? 0,
    postLastPromptDelayMs: session.postLastPromptDelayMs ?? 120000,
    maxPromptsPerRun: session.maxPromptsPerRun ?? 10,
    autoChainAfterPrompts: session.autoChainAfterPrompts ?? false,
  };
}
